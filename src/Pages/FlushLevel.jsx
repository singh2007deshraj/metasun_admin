...
function FlushLevel() {
  ...
  const fetchData = async (currentPage, query, limit, fromTime, toTime) => {
    try {
      setLoading(true);
      const response = await getUsers("flush-income", {
        page: currentPage,
        limit,
        query,
        type: "level",
      ...
      <div className="container-fluid mt-4 bg-white p-4 rounded shadow-sm overflow-auto">
        <div className="d-flex flex-wrap align-items-center justify-content-between mb-4">
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
            {/* <div>
            <h3 className="mb-0">Flush Level Income Report</h3>
          </div> */}
                  ...
                  {data?.length === 0 || !data ? (
                    ...
                    data.map((entry, index) => (
                      <tr key={index} className="text-center">
                        ...
                        <td>
                          {packageName[entry?.fromPackageId - 1] || "N/A"}
                        </td>
                        <td>
                        ...
                        <td
                          title={`$${Number(
                            (price * Number(entry?.flushIncome)) / 1e18
                          ).toFixed(4)}`}
                        >
                          {entry?.flushIncome ? "BTC" : ""}{" "}
                          {entry?.flushIncome
                            ? (Number(entry.flushIncome) / 1e18).toFixed(7)
                            : "N/A"}
                        </td>
                        <td
                          title={`$${Number(
                            (price * Number(entry?.finalFlushIncome)) / 1e18
                          ).toFixed(4)}`}
                        >
                          {entry?.finalFlushIncome ? "BTC" : ""}{" "}
                          {entry?.finalFlushIncome
                            ? (Number(entry.finalFlushIncome) / 1e18).toFixed(7)
                            : "N/A"}
                        </td>
                        <td>
