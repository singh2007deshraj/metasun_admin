...
function LevelIncome() {
  ...
  const fetchData = async (currentPage, query, limit, fromTime, toTime) => {
    try {
      setLoading(true);
      const response = await getUsers("level-income", {
        page: currentPage,
        limit,
        query,
                  ...
                  {data?.length === 0 || !data ? (
                    ...
                    data.map((entry, index) => (
                      <tr key={index} className="text-center">
                        ...
                        <td>
                          <span className="d-inline-flex align-items-center gap-3">
                            ...
                            <button
                              ...
                              title="Copy"
                              onClick={() => handleCopy(entry?.fromUserId)}
                            >
                        ...
                        <td>{packageName[entry?.fromPackage - 1] || "-"}</td>
                        <td>
                          {packageName[entry?.receiverPackageId - 1] || "-"}
                        </td>
                        <td>
