...
function PayoutWithdrawal() {
  ...
  const getPayoutWithdrawal = async () => {
    try {
      setLoading(true);
  ...
  const handleAction = async (action, requests) => {
    try {
      ...
      const res = await axios.post(
        `${base_url}/admin/approve-payouts`,
        { action, requests },
        ...
        {error ? (
          ...
          <>
            ...
            {activeTab === "pending" ? (
                ...
                <div className="mb-3 p-3 d-flex align-items-end justify-content-between flex-wrap bg-white rounded">
                  ...
                  {/* <div>
                    <FormControl
                      ...
                      sx={{ width: "150px" }}
                    >
                      <InputLabel id="select-passkey-label">
                        Select PassKey
                      </InputLabel>
                      <Select
                        labelId="select-passkey-label"
                        value={filterData}
                        onChange={(e) => setFilterData(e.target.value)}
                        label="Select PassKey"
                      >
                        <MenuItem value="all">All</MenuItem>
                        {passKeyList?.map((pass) => (
                          <MenuItem
                            key={pass?.passKeyInfo?.passKey}
                            value={pass?.passKeyInfo?.passKey}
                          >
                            {pass?.passKeyInfo?.passKey}
                          </MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                      ...
                      {loading ? (
                        ...
                        swapData?.map((user, index) => (
                          <tr key={index}>
                            ...
                            <td>{user.userId}</td>
                            <td>${Number(user?.requestedAmount/1e18 || 0).toFixed(2)}</td>
                            <td>{user?.deductionPercent*100}%</td>
                            <td>${Number(user?.deductionAmount/1e18 || 0).toFixed(2)}</td>
                            <td>${Number(user?.finalAmount/1e18 || 0).toFixed(2)}</td>
              ...
              <>
              <ApprovedCompoundWithdrawal url="payout-requests"/>
              </>
            ) : (
              <>
                <RejectedWithdrawal url="payout-requests"/>
              </>
            )}
          </>
        )}
