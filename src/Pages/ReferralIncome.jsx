...
function ReferralIncome() {
  ...
  const fetchData = async (currentPage, query, limit, fromTime, toTime) => {
    try {
      setLoading(true);
      const response = await getUsers("referral-income", {
        page: currentPage,
        limit,
        query,
      ...
      <div className="container-fluid mt-4 bg-white p-4 rounded shadow-sm overflow-auto">
        <div className="d-flex flex-wrap align-items-center justify-content-between mb-4">
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
            {/* <div>
            <h3 className="mb-0">Referral Income Report</h3>
          </div> */}
                  ...
                  {data?.length === 0 || !data ? (
                    ...
                    data.map((entry, index) => (
                      <tr key={index} className="text-center">
                        ...
                        <td>{entry?.fromLevel ?? "-"}</td>
                        <td>{packageName[entry?.fromPackage - 1] || "N/A"}</td>
                        <td>
                        ...
                        <td
                          title={`$${Number(
                            (price * Number(entry?.finalAmount)) / 1e18
                          ).toFixed(4)}`}
                        >
                          BTC{" "}
                          {entry?.finalAmount
                            ? (Number(entry.finalAmount) / 1e18).toFixed(7)
                            : "N/A"}
                        </td>
                        <td>
